...
| `material.inc.php` | Game material description (cards, tokens) — **optional**; not generated by the scaffold |
...
| Backend API | Autowired `act*` methods in `Game.php` / `States/` (no `action.php` required; `action.php` is legacy, optional) | Routes to `act*` methods |
