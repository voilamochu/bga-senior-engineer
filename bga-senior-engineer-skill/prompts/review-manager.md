...
- rules/persistence.json — Load when reviewing Manager database access patterns. Check PERS-001 through PERS-014.

## Workflow

### Step 1: Load the Canonical Example

Load examples/manager-example.json to see the standard Manager pattern. Review the annotations to understand why each element exists.
...
If the Manager contains SQL queries, load rules/persistence.json and check PERS-001 through PERS-014 for correct schema design, atomic UPDATE patterns, and proper transaction boundaries.
