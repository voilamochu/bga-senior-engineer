...
- rules/actions.json — Load when debugging action handler behavior. Check ACTN-001 through ACTN-013.
- rules/notifications.json — Load when debugging notification delivery or payload. Check NOTF-001 through NOTF-014.
- rules/state-machine.json — Load when debugging state transitions or zombie mode. Check STAT-001 through STAT-016.
- rules/architecture.json — Load when debugging component boundaries or ownership. Check ARCH-001 through ARCH-022.
- rules/persistence.json — Load when debugging database queries or globals. Check PERS-001 through PERS-014.
...
If the bug is in an action handler, load rules/actions.json. If the bug involves notifications, load rules/notifications.json. If the bug is in state transitions, load rules/state-machine.json. For component ownership issues, load rules/architecture.json. For database issues, load rules/persistence.json.
...
If undo or replay logic is involved, load rules/undo-replay.json and check UNDO-001 through UNDO-005.
