...
lazy_rules:
  - references/rules/synchronization.json
required_examples:
...
- references/rules/synchronization.json — Load when reviewing spectator or reconnect notification paths. Check SYNC-002 through SYNC-005.

## Workflow

### Step 1: Load the Canonical Example

Load assets/examples/notification-example.json to see the standard notification pattern. Review the annotations.
...
If reviewing spectator or reconnect paths, load references/rules/synchronization.json and check SYNC-002 through SYNC-005.
