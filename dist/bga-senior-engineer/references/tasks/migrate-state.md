...
- references/rules/migration.json — Load when handling legacy states.inc.php patterns. Check MIGR-007.

## Workflow

### Step 1: Load the Canonical Example

Load assets/examples/state-example.json to see the standard State class structure.
...
If the legacy code uses numeric state IDs inline or non-semantic transition keys, load references/rules/migration.json and check MIGR-007 for migration guidance.
