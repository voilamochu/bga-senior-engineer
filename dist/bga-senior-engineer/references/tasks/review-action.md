...
- references/rules/undo-replay.json — Load when reviewing actions that mutate undoable state. Check UNDO-001 through UNDO-005.

## Workflow

### Step 1: Load the Canonical Example

Load assets/examples/action-example.json to see the standard action handler pattern. Review the annotations to understand why each element exists.
...
1. Load references/rules/undo-replay.json
