...
- references/rules/persistence.json — Load when extracting SQL from Game.php. Check PERS-001 through PERS-005.
- references/rules/migration.json — Load when planning extraction sequence. Check MIGR-001 through MIGR-014.
...
Load assets/examples/manager-example.json to see the standard Manager structure. Load assets/examples/model-example.json to see the standard Model pattern.
...
If the legacy Game.php contains SQL, load references/rules/persistence.json and check PERS-001 through PERS-005 for atomic conditional UPDATE patterns.
...
If the extraction is part of a larger migration, load references/rules/migration.json and plan the sequence following MIGR-014 (config files first, then Managers, then state machine). Extract one concern per commit following MIGR-009.
