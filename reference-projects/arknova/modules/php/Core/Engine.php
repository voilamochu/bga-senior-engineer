...
class Engine
{
  ...
  public static function proceed($confirmedPartial = false, $isUndo = false)
  {
    ...
    if ($node == null) {
      ...
      if (Globals::getEngineChoices() == 0 && !$player->hasAnytimeUsefulAction()) {
  ...
  public static function checkCanTakeIrreversible()
  {
    ...
    if (!is_null($node) && (is_null($node->getPId()) || $node->getPId() == $player->getId())) {
      $msg = totranslate(
      ...
      if (!is_null($action)) {
        if ($action == VENOM_PAY) {
          $msg = totranslate('You can\'t take this irreversible action because you don\'t have enough money to pay for Venom.');
        } elseif ($action == ANIMALS) {
          $msg = totranslate('You can\'t take this irreversible action because you can\'t play any Animal.');
        } elseif ($action == BUILD) {
          $msg = totranslate('You can\'t take this irreversible action because you can\'t place the unique building of a Sponsor you played.');
        }
      }
  ...
  public static function getAfterFinishingNode()
  {
    ...
    // Are there any pending VENOM at the end of the action ??
    $lastNode = $childs[count($childs) - 1];
    if ($lastNode->getAction() == VENOM_PAY) {
      $node = self::buildTree($t);
      self::$tree->insertChildAtPos($node, count($childs) - 2);
      self::save();
      return $node;
    }
    // Otherwise, just insert at the end
    else {
      return self::insertAtRoot($t);
    }
  }
