...
class Log extends \APP_DbObject
{
  ...
  public static function getUndoableSteps($onlyIds = true, $includeEngineStarts = false)
  {
    $checkpoint = self::getLastCheckpoint($includeEngineStarts);
    $query = new QueryBuilder('log', null, 'id');
    $logs = $query
      ->select(['id', 'move_id'])
      ->where('type', 'step')
  ...
  {
    $lastStep = static::getUndoableSteps(false, true)->last();
    if (is_null($lastStep)) {
      throw new \BgaVisibleSystemException('Nothing to undo!');
    }
    return self::revertTo($lastStep['id']);
  }
  ...
  public static function revertTo($id)
  {
    ...
    foreach ($logs as $log) {
      ...
      $moveId = intval($log['move_id']);
      if (!in_array($moveId, $moveIds)) {
        $moveIds[] = $moveId;
      }
