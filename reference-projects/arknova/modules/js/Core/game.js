...
define(['dojo', 'dojo/_base/declare', g_gamethemeurl + 'modules/js/vendor/nouislider.min.js', 'ebg/core/gamegui'], (
  ...
  return declare('customgame.game', ebg.core.gamegui, {
    ...
    setupNotifications() {
      ...
      this._notifications.forEach((notif) => {
        ...
        let wrapper = async (args) => {
          let multiactive = this.gamedatas && this.gamedatas.gamestate && this.gamedatas.gamestate.type == 'multipleactiveplayer';
          if (!multiactive) {
            this.clearTitleBar();
          }
     ...
     */
    addSafeActionButton(id, text, callback, zone, color) {
      if ($(id)) return;

      let safeCallback = () => {
        if ($(id) && $(id).classList.contains('disabled')) return;
        callback();
      };

      this.addActionButton(id, text, safeCallback, zone, false, color);
    },

    addPrimaryActionButton(id, text, callback, zone = 'customActions') {
      this.addSafeActionButton(id, text, callback, zone, 'blue');
    },

    addSecondaryActionButton(id, text, callback, zone = 'customActions') {
      this.addSafeActionButton(id, text, callback, zone, 'gray');
    },

    addDangerActionButton(id, text, callback, zone = 'customActions') {
      this.addSafeActionButton(id, text, callback, zone, 'red');
    },
