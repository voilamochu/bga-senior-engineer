...
trait GameStatesTrait
{
    public function eventPlay()
    {
        ...
        if (!$cardMgr->playerHasAnytimeEventInHand($playerId)) {
            throw new \BgaUserException(clienttranslate('You do not have any event card to play'));
        }
    ...
    public function eventPay(array $payedSproutList, array $payedGrowthList, array $payedCompostFromHandCardIds)
    {
        ...
        $this->addInstantEventGain($creator, $cardId);
    ...
    private function addInstantEventGain(\BX\Action\ActionCommandCreatorInterface $creator, int $eventCardId)
    {
        $instantEventGainPlayerId = $creator->getPlayerId();
        $cardMgr = \BX\Action\ActionRowMgrRegister::getMgr('card');

        // For End of Turn Events, only apply the gain for the player that plays the event
        $eventCardDef = \EA\CardDefMgr::getByCardId($eventCardId);
        if ($eventCardDef === null) {
            throw new \BgaSystemException("BUG! No card def for eventCardId $eventCardId");
        }
        if ($eventCardDef->isEndTurnEvent()) {
            $eventCard = $cardMgr->getCardById($eventCardId);
            if ($eventCard === null) {
                throw new \BgaSystemException("BUG! No card for eventCardId $eventCardId");
            }
            if ($eventCard->playerId != $instantEventGainPlayerId) {
                return;
            }
        }

        $card = $cardMgr->getPlayerIslandCard($instantEventGainPlayerId);
        $cardId = $card->cardId;
        $abilityBrown = $card->getCardDef()->abilityBrown();
        if ($abilityBrown === null) {
            return;
    ...
    private function commonPlayEvent(int $playerId, int $cardId, callable $playFct, callable $nextStateFct, bool $forceCommit = false)
    {
        ...
        if ($ability->hasUserPlacementPayment()) {
            $creator = \BX\Action\buildActionCommandCreator($playerId, $forceCommit);
            $playFct($creator);
            ...
            $creator = \BX\Action\buildActionCommandCreator($playerId, $forceCommit || $ability->mustGainCommit() || $ability->mustPaymentCommit() || $this->eventWillDrawCard($playerId));
            $playFct($creator);
            $creator->add(new \EA\Actions\Activation\ForceCardActivation($playerId, $cardId));
            $this->addInstantPayment($cardId, $creator, $ability);
            $this->addPlacementGain($cardId, $creator, $ability, null);
