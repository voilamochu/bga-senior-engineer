...
trait GameStatesTrait
{
    public function stGameNextPhase()
    {
        ...
        foreach ($playerIdArray as $playerId) {
            ...
            if (gameHasExpansionAbundance() && $cardMgr->isTableauFilledForPlayer($playerId)) {
                if (!$leafTokenMgr->hasFaunaBoardTableauBonus()) {
                    \BX\Action\ActionCommandMgr::saveOneAndCommit(new \BX\Action\SendMessage(
                        $playerId,
                        clienttranslate('${player_name} cannot claim the 4x4 tableau bonus on the fauna board: they don\'t have any leaf token left or are not the active player.'),
                    ));
                }
                break;
            }
