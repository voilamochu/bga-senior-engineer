...
trait GameStatesTrait
{
    ...
    public function stEnterEndTurn()
    {
        ...
        if (!$playerStateMgr->hasAllSameLastSeenEndTurnEvents()) {
            foreach ($playerIdArray as $playerId) {
                if ($playerMgr->isPlayerZombie($playerId)) {
                    continue;
                }
    ...
    public function stEndTurn()
    {
        ...
        $this->gamestate->setPlayersMultiactive($activatePlayerIds, '', true /*Exclusive*/);
    ...
    public function endTurnPlayEndTurnEvent()
    {
        ...
        $cardMgr = \BX\Action\ActionRowMgrRegister::getMgr('card');
        if (!$cardMgr->playerHasEndTurnEventInHand($playerId)) {
            throw new \BgaUserException(clienttranslate('You do not have any end turn event card to play'));
        }
    ...
    public function endTurnChooseEndTurnEvent(int $cardId)
    {
        ...
        if (
            gameVersionHasEndOfTurnEventPlayPayment()
            ...
            $this->commonPlayEvent(
                $playerId,
                $cardId,
                function ($creator) use ($playerId, $cardId) {
                    $creator->add(new \EA\Actions\Event\PlayEventCard($playerId, $cardId));
                    if (gameVersionHasEndOfTurnEventPlayPayment()) {
                        $creator->add(new \EA\Actions\EndTurn\ChooseEndTurnEventCard($playerId, $cardId, false));
                    }
                },
                ...
                gameVersionHasEndOfTurnEventPlayPayment() /* force commit */
            );
        } else {
            $creator = new \BX\Action\ActionCommandCreatorCommit($playerId);
    ...
    public function argsEndTurnEventChoose(int $playerId)
    {
        return \BX\MultiActiveState\argsMultiActive($playerId, function ($playerId) {
            ...
            $skipPlayerId = null;
            if (count($cards) > 0 && gameVersionHasEndOfTurnEventPlayPayment()) {
                $skipPlayerId = $cards[0]->playerId;
            }
            $message = clienttranslate('Activate the end turn event or Pass');
            if ($skipPlayerId !== null && $skipPlayerId == $playerId) {
                $message = clienttranslate('Pass (you have already activated this event)');
            }
            $ret = [
                'canPlaceExchangeSprout' => $playerExchangeMgr->getPlayerSproutCount($playerId) > 0
                    && $cardMgr->getPlayerUnusedSproutSpaceCount($playerId) > 0,
                ...
                'endTurnEventChoose' => $message,
                'i18n' => ['endTurnEventChoose'],
            ];
            return $this->argsMergeEarthDefault($playerId, $ret);
        });
    }
    ...
    public function endTurnEventActivate()
    {
        ...
        if (
            ...
            throw new \BgaSystemException('BUG! Activating end turn event but player is the one who played it!');
        }
