...
trait GameActionsTrait
{
    ...
    public function argPrivateStateArgs()
    {
        ...
        $publicStates = $this->gamestate->getStatesAsArray();
        $publicState = null;
        ...
        foreach (array_keys($this->loadPlayersBasicInfos()) as $playerId) {
            ...
            if ($privateState->stateId === null) {
                ...
                if (!array_key_exists($privateState->stateId, $this->gamestate->getStatesAsArray())) {
                ...
                if (array_key_exists('args', $this->gamestate->getStatesAsArray()[$privateState->stateId])) {
                    $func = $this->gamestate->getStatesAsArray()[$privateState->stateId]['args'];
                    if ($func !== null) {
    ...
    public function privateStateCheckAction($playerId, $actionName, $playerActiveStatus)
    {
        ...
        $states = $this->gamestate->getStatesAsArray();
        if (!\array_key_exists($stateId, $states)) {
            throw new \BgaSystemException("BUG! State {$stateId} does not exists");
        }
        $state = $states[$stateId];
