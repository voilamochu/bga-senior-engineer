...
class SpecialEffect extends \AGR\Models\Action
{
  ...
  public function stSpecialEffect()
  {
    ...
    if (\method_exists($card, $method)) {
      $checkpoint = $card->$method(...$arguments) ?? false;
      $this->resolveAction([], $checkpoint);
    }
  }
  ...
  public function actSpecialEffect(...$actArgs)
  {
    ...
    // Irreversible check
    $checkpoint = false;
    $method = 'is' . \ucfirst($args['method']) . 'Irreversible';
    $arguments = $args['args'] ?? [];
    if (\method_exists($card, $method)) {
      $checkpoint = $card->$method(...array_merge($actArgs, $arguments));
    }
