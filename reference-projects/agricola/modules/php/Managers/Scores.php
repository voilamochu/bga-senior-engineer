...
class Scores extends \AGR\Helpers\DB_Manager
{
  ...
  // Cards that score from resources (Pottery/Large Pottery/Joinery/Basket) reserve here what they consume so a second card
  // on the same pile can't recount it.
  protected static $liveReserved = [];

  public static function reserveForScoring($pId, $type, $amount)
  {
    if ($amount > 0) {
      self::$liveReserved[$pId][$type] = (self::$liveReserved[$pId][$type] ?? 0) + $amount;
    }
  }

  public static function getLiveReservedForScoring($pId, $type)
  {
    return self::$liveReserved[$pId][$type] ?? 0;
  }
   ...
   */
  public static function getQtyDesc($player, $n, $score, $descSingular, $descPlural)
  {
    $desc = [
      'log' => '',
      'args' => [
  ...
  public static function compute()
  {
    ...
    foreach (Players::getAll() as $pId => $player) {
      ...
      self::computeC31($player, self::$scores);
      self::computeA39($player, self::$scores);
    }
  ...
  protected static function computeFields($player)
  {
  ...
  protected static function computeSheeps($player)
  {
  ...
  protected static function computePigs($player)
  {
  ...
  protected static function computeCattles($player)
  {
  ...
  protected static function computeFencedStables($player)
  {
  ...
  protected static function computeRooms($player)
  {
    $n = $player->countRooms();
  ...
  protected static function computeFarmers($player)
  {
    $n = $player->countFarmers();
    $E159tax = $player->hasPlayedCard('E159_OldMiser');
    $score = (3-$E159tax) * $n;
  ...
  protected static function computeCards($player)
  {
  ...
  protected static function computeC135($player, $score)
  {
  ...
  protected static function computeD100($player, $score)
  {
    if ($player->hasPlayedCard('D100_LordoftheManor')) {
      PlayerCards::get('D100_LordoftheManor')->computeSpecialScore($score);
    }
  }

  protected static function computeC31($player, $score)
  {
    if ($player->hasPlayedCard('C31_WritingChamber')) {
      PlayerCards::get('C31_WritingChamber')->computeSpecialScore($score);
    }
  }

  protected static function computeA39($player, $score)
  {
    if ($player->hasPlayedCard('A39_Chapel')) {
      PlayerCards::get('A39_Chapel')->computeSpecialScore($score);
    }
  }
