...
class QueryBuilder extends \APP_DbObject
{
  ...
  public function values($rows = [])
  {
    ...
    foreach ($rows as $row) {
      ...
      $ids[] =
        $row[$this->primary] ?? ($this->insertPrimaryIndex === false ? null : $row[$this->insertPrimaryIndex]);
    }

    $this->sql .= implode(',', $vals);
    self::DbQuery($this->sql);
    if ($this->insertPrimaryIndex === false && count($rows) > 0) {
      $firstInsertedId = (int) self::DbGetLastId();
      if ($firstInsertedId > 0) {
        $ids = [];
        for ($i = 0; $i < count($rows); $i++) {
          $ids[] = $firstInsertedId + $i;
        }
      }
    }
    if ($this->log) {
