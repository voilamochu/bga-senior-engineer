...
class XorNode extends AbstractNode
{
  ...
  {
    $hasDoableChild = $this->childsReduceOr(function ($child) use ($player, $ignoreResources) {
      return $child->isDoable($player, $ignoreResources);
    });

    if ($this->infos['doableRequiresChild'] ?? false) {
      return $hasDoableChild;
    }

    return $this->isOptional() || $hasDoableChild;
  }
