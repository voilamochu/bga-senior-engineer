    ...
    if (preg_match('/^([gs]et|inc)([A-Z])(.*)$/', $method, $match)) {
      ...
      if ($match[1] == 'get') {
        ...
        if (is_object($args[0])) {
          ...
          $player = Players::get($args[0]);
        }
        if ($player->hasPlayedCard('D36_BreedRegistry') && ($name == 'cardsSheep' || $name == 'boardSheep')) {
          $n = Self::getBoardSheep($player) + Self::getCardsSheep($player);
          PlayerCards::get('D36_BreedRegistry')->updateInfobox($n . ' / 2');
        }
