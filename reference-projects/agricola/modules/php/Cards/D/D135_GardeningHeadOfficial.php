...
class D135_GardeningHeadOfficial extends \AGR\Models\Occupation
{
  public function __construct($row)
  {
    parent::__construct($row);
    $this->id = 'D135_GardeningHeadOfficial';
    $this->name = clienttranslate('Gardening Head Official');
    $this->deck = 'D';
    $this->number = 135;
  ...
  public function computeBonusScore()
  {
    ...
    foreach (Players::getAll() as $player) {
      $fieldVegetables = $player->board()->getGrowingCrops(VEGETABLE)->count();

      if ($fieldVegetables > $max) {
        $max = $fieldVegetables;
        $bonusPlayers = [];
      }
