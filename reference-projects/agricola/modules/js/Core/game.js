  ...
  return declare('customgame.game', ebg.core.gamegui, {
    ...
    unsetModeInstantaneous() {
      if (this.instantaneousMode) {
        ...
        dojo.style('leftright_page_wrapper', 'display', '');
    ...
    onEnteringState(stateName, args) {
      ...
      this._instantTitleBarTransition();
    ...
    _instantTitleBarTransition() {
      if (!this.isInstantSpeed()) return;
      let bar = $('page-title');
      if (!bar) return;
      bar.classList.add('instant-transitioning');
      clearTimeout(this._titleBarDebounce);
      this._titleBarDebounce = setTimeout(() => {
        bar.classList.remove('instant-transitioning');
      }, 30);
    },
    ...
    setupNotifications() {
      ...
      this._notifications.forEach((notif) => {
        ...
        let wrapper = (args) => {
          ...
          if (msg != '') {
            this.setTitleBarText(msg);
          }

          let timing;
          try {
            timing = this[functionName](args);
          } catch (e) {
            // A thrown handler must never wedge the synchronous queue (permanent 'Updating game' lock).
            // Re-throw async so the error still reaches BGA's error telemetry as uncaught.
            console.error('Error in notification handler ' + notif[0], e);
          ...
          // If handler returned an explicit long timing in instant mode,
          // show the title bar immediately (e.g. harvest animation)
          if (this.isInstantSpeed() && timing > 0) {
            clearTimeout(this._titleBarDebounce);
            let bar = $('page-title');
            if (bar) bar.classList.remove('instant-transitioning');
          }
          ...
          if (timing === undefined) {
            ...
            // Override default timing by 0 in case of fast replay mode, otherwise scale by animation speed
            // null means "handler controls queue via slideResources" — preserve it
            timing = this.isFastMode() ? 0 : (notif[1] === null ? null : Math.round(notif[1] * this.getAnimationSpeedMultiplier()));
          }
    ...
    slide(mobileElt, targetElt, options = {}) {
      ...
      // Handle fast mode / instant speed
      if ((this.isFastMode() || this.isInstantSpeed()) && (config.destroy || config.clearPos)) {
        if (config.destroy) dojo.destroy(mobile);
        else if (config.beforeBrother != null) dojo.place(mobile, config.beforeBrother, 'before');
      ...
      // Scale duration by animation speed
      config.duration = Math.round(config.duration * this.getAnimationSpeedMultiplier());
    ...
    flipAndReplace(target, newNode, duration = 1000) {
      // Fast replay mode or instant speed
      ...
      // Scale duration by animation speed
      duration = Math.round(duration * this.getAnimationSpeedMultiplier());
    ...
    /**
     * Force-close a card's tooltip and cancel any pending open timeout.
     */
    closeTooltip(cardId) {
    ...
    addCustomTooltip(id, html, delay) {
      ...
      // On touch devices, mouseenter fires synthetically after touchstart/touchend.
      // Track this so mouseenter can skip opening the tooltip (mouseleave never fires on touch).
      let isTouchEvent = false;
      dojo.connect($(id), 'touchstart', () => {
        isTouchEvent = true;
        this.closeTooltip(id);
      });
   ...
   addLogClass() {
    ...
    createCounter(id, defaultValue = 0) {
      ...
      let o = {
        ...
        toValue: function (n) {
          if (game.isFastMode() || game.isInstantSpeed()) {
            this.setValue(n);
            return;
          }
